...
2. Installs WiX Toolset v3 via Chocolatey; the MSI build step auto-detects
   the actual `candle.exe` / `light.exe` path at runtime (PATH → common
   install dirs → recursive search), so no path is hardcoded.
